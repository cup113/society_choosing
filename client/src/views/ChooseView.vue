  ...
  <main class="flex flex-col gap-4">
    ...
    <div>
      <div class="fixed bottom-0 border-2 shadow-lg bg-[#fde1ba] w-full flex items-center justify-center pt-4 pb-2 gap-4 md:gap-8 lg:gap-12 md:px-8">
        ...
        <Button v-if="societyStore.timeStatus?.open" @click="submit()" class="submit-btn relative w-20 my-8 bg-amber-700 hover:bg-amber-800">提交</Button>
      </div>
    </div>
    <Waiting :show="waiting">
      <div>正在提交选课...</div>
    </Waiting>
    <ConfirmChoice :open="waiting_confirm" @confirm-choice="submit_confirmed" @cancel-choice="waiting_confirm = false" :choice="choiceName"></ConfirmChoice>
  </main>
</template>
